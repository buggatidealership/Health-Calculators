...
// DERMAMEDICUM GLITCH: PhilArt (PDRN) myth-buster
// Pink influencer claim → digital glitch corruption → dark clinical truth
...
export const DM_PhilArt_Glitch: React.FC = () => {
  ...
  const isGlitch = frame >= 120 && frame < 165;
  const isTruth = frame >= 165 && frame < 280;
  const isCTA = frame >= 280;

  // Glitch intensity
  const glitchIntensity = isGlitch
    ? interpolate(frame, [120, 138, 155, 165], [0, 1, 1, 0], clamp)
    : 0;

  // Flicker between pink and dark
  const flickerToDark = isGlitch
    ? seeded(Math.floor(frame / 2), 1) < interpolate(frame, [120, 160], [0.1, 0.9], clamp)
    : frame >= 165 ? 1 : 0;
  ...
  const mythContent = (
    <div style={{
      ...
      <div style={{
        ...
        {"\u201E"}Hyaluron ist das Beste f{"\u00FC"}r Anti-Aging!{"\u201C"}
      </div>
      <div style={{
        fontSize: 36,
        color: "#ad1457",
        ...
        {"\uD83D\uDC89\u2728"} 287K Likes {"\u00B7"} Skincare-Influencerin
      </div>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      {/* PHASE 2: GLITCH ARTIFACTS (120-165) */}
      {isGlitch && (
        <>
          ...
          <div style={{
            ...
            <div style={{ position: "absolute", inset: 0, background: "rgba(255, 0, 0, 0.4)" }} />
          </div>
          ...
          <div style={{
            ...
            <div style={{ position: "absolute", inset: 0, background: "rgba(0, 80, 255, 0.4)" }} />
          </div>
          ...
          {tearBars.map((bar, i) => (
            <div key={i} style={{
              position: "absolute",
              top: bar.top, left: 0,
              width: SPEC.width, height: bar.height,
          ...
          <div style={{
            position: "absolute", inset: 0,
            backgroundImage: `repeating-linear-gradient(0deg, transparent, transparent 3px, rgba(0, 0, 0, ${scanlineOpacity}) 3px, rgba(0, 0, 0, ${scanlineOpacity}) 4px)`,
          ...
          {seeded(frame, 77) > 0.7 && (
            <div style={{
              position: "absolute", inset: 0,
              background: seeded(frame, 88) > 0.5 ? "rgba(74, 222, 128, 0.15)" : "rgba(255, 255, 255, 0.2)",
      ...
      {isTruth && (
        <div style={{
          ...
          <div style={{
            ...fadeIn(168),
            width: interpolate(frame, [168, 200], [0, 800], clamp),
            height: 4,
          ...
          <div style={{
            ...
            Polynukleotide aktivieren Fibroblasten und stimulieren k{"\u00F6"}rpereigene Kollagenproduktion {"\u2014"} ein grundlegend anderer Ansatz.
          </div>
          ...
          <div style={{
            ...
            Quelle: Polynucleotides in Aesthetic Medicine, MDPI 2024
          </div>
        </div>
      )}
      ...
      {isCTA && (
        <div style={{
          ...
          <div style={{
            ...
            Regeneration statt Auff{"\u00FC"}llung.
          </div>
