document.addEventListener('DOMContentLoaded', function() {
  ...
  waterVolumeInput.addEventListener('change', function() {
  ...
  calculateBtn.addEventListener('click', function() {
    var vialIU;
    if (vialStrengthInput.value === 'custom') {
      vialIU = parseFloat(customVialInput.value);
    } else {
    ...
    var waterMl;
    if (waterVolumeInput.value === 'custom') {
      waterMl = parseFloat(customWaterInput.value);
    } else {
      waterMl = parseFloat(waterVolumeInput.value);
    }
    ...
    // Calculations
    var concentration = vialIU / waterMl; // IU per mL
    var injectionMl = desiredDose / concentration;
    var syringeUnits = injectionMl * 100; // U-100 insulin syringe
    ...
    document.getElementById('dose-label').textContent = desiredDose.toLocaleString() + ' IU per injection';
    ...
    if (injectionMl < 0.05) {
      ...
      iconEl.textContent = '\u26A0';
      descEl.textContent = 'Warning: This injection volume (' + injectionMl.toFixed(3) + ' mL = ' + syringeUnits.toFixed(1) + ' units) is very small and difficult to measure accurately. Consider using more bacteriostatic water to increase the volume per dose. For example, adding ' + (waterMl * 2) + ' mL instead of ' + waterMl + ' mL would double the injection volume.';
    } else {
      statusEl.className = 'result-status status-good';
      iconEl.textContent = '\u2713';
      descEl.textContent = 'Draw ' + injectionMl.toFixed(2) + ' mL (' + syringeUnits.toFixed(1) + ' units on a U-100 insulin syringe) for each ' + desiredDose.toLocaleString() + ' IU injection. This vial provides ' + dosesPerVial + ' doses at this amount.';
    }

    // Comparison boxes
    document.getElementById('comp-volume').textContent = injectionMl.toFixed(2) + ' mL';
    document.getElementById('comp-units').textContent = syringeUnits.toFixed(1) + ' units';
    document.getElementById('comp-doses').textContent = dosesPerVial;

    // Breakdown
    document.getElementById('display-concentration').textContent = concentration.toLocaleString() + ' IU/mL';
    document.getElementById('display-injection-ml').textContent = injectionMl.toFixed(3) + ' mL';
    document.getElementById('display-syringe-units').textContent = syringeUnits.toFixed(1) + ' units (U-100 insulin syringe)';
    document.getElementById('display-doses').textContent = dosesPerVial + ' doses per vial';
    document.getElementById('display-waste').textContent = wastedIU > 0 ? wastedIU.toLocaleString() + ' IU remaining (not enough for a full dose)' : 'None \u2014 vial divides evenly';
    document.getElementById('display-vial').textContent = vialIU.toLocaleString() + ' IU vial + ' + waterMl + ' mL bacteriostatic water';
