document.addEventListener('DOMContentLoaded', function() {
  ...
  date2Input.value = formatDate(now);
  date1Input.value = formatDate(twoDaysAgo);
  time1Input.value = '08:00';
  time2Input.value = '08:00';
  ...
  calculateBtn.addEventListener('click', function() {
    ...
    var d1 = date1Input.value;
    var d2 = date2Input.value;
    var t1 = time1Input.value || '08:00';
    var t2 = time2Input.value || '08:00';
    ...
    // Formula: doublingTime = (hours * ln(2)) / ln(hcg2 / hcg1)
    var ratio = hcg2 / hcg1;
    ...
    if (ratio === 1) {
      doublingTimeHours = Infinity; // no change
    } else {
    ...
    if (isDeclining) {
      heroEl.textContent = 'Declining';
      document.getElementById('doubling-label').textContent = 'hCG is decreasing';
      statusEl.className = 'result-status status-danger';
      iconEl.textContent = '⚠';
      descEl.textContent = 'hCG levels are falling — this may indicate a non-viable pregnancy, miscarriage, or ectopic pregnancy. Contact your healthcare provider.';
    } else if (!isFinite(doublingTimeHours) || doublingTimeHours <= 0) {
      heroEl.textContent = 'N/A';
      document.getElementById('doubling-label').textContent = 'Unable to calculate';
      statusEl.className = 'result-status status-warning';
      iconEl.textContent = '⚠';
      descEl.textContent = 'hCG levels are unchanged or the values entered may be incorrect.';
    } else {
      var displayHours = Math.round(doublingTimeHours);
      heroEl.textContent = displayHours + ' hrs';
      document.getElementById('doubling-label').textContent = 'hCG Doubling Time';
      ...
      statusEl.className = 'result-status status-' + category.statusClass;
      iconEl.textContent = category.icon;
      descEl.textContent = category.text;
    }
    ...
    if (isRising && isFinite(doublingTimeHours) && doublingTimeHours > 0) {
      ...
      document.getElementById('gauge-section').classList.remove('hidden');
    } else {
      document.getElementById('gauge-section').classList.add('hidden');
    }

    // Breakdown values
    document.getElementById('display-hcg1').textContent = formatNumber(hcg1) + ' mIU/mL';
    document.getElementById('display-hcg2').textContent = formatNumber(hcg2) + ' mIU/mL';
    document.getElementById('display-change').textContent = (pctChange >= 0 ? '+' : '') + pctChange.toFixed(1) + '%';
    document.getElementById('display-change').style.color = isDeclining ? '#dc2626' : '#16a34a';

    document.getElementById('display-hours').textContent = diffHours.toFixed(1) + ' hours (' + (diffHours / 24).toFixed(1) + ' days)';

    if (isRising && isFinite(twoDayPct)) {
      document.getElementById('display-48h').textContent = '+' + twoDayPct.toFixed(1) + '% per 48 hours';
      document.getElementById('row-48h').classList.remove('hidden');
    } else {
      document.getElementById('row-48h').classList.add('hidden');
    }

    if (isRising && isFinite(doublingTimeHours) && doublingTimeHours > 0) {
      document.getElementById('display-doubling-detail').textContent = doublingTimeHours.toFixed(1) + ' hours (' + (doublingTimeHours / 24).toFixed(1) + ' days)';
      document.getElementById('row-doubling').classList.remove('hidden');
    } else {
      document.getElementById('row-doubling').classList.add('hidden');
    }

    // Comparison boxes
    document.getElementById('comp-hcg1').textContent = formatNumber(hcg1);
    document.getElementById('comp-hcg2').textContent = formatNumber(hcg2);
    document.getElementById('comp-diff').textContent = (pctChange >= 0 ? '+' : '') + pctChange.toFixed(1) + '%';
    ...
    if (typeof showNextSteps === 'function') {
      showNextSteps('hcg-doubling', {});
    }
  });

  function getDoublingCategory(hours, hcg1Level) {
    // Normal doubling times vary by hCG level:
    // <1,200 mIU/mL: 48-72 hours expected
    // 1,200-6,000: 72-96 hours expected
    // >6,000: >96 hours (rate slows naturally)
    if (hcg1Level < 1200) {
      if (hours <= 72) {
      ...
      // >6,000 — rate is naturally much slower
      if (hours <= 120) {
