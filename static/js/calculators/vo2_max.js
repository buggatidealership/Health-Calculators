// VO2 Max Calculator — factory-migrated
(function() {
  ...
  function fitnessAge(vo2, sex) {
    ...
    return midpoints[midpoints.length - 1] + 5;
  }
  ...
  function calculate() {
    ...
    switch (method) {
      case 'cooper':
        ...
        var distUnit = (document.getElementById('cooper-unit') || {}).value || 'miles';
        age = parseInt((document.getElementById('cooper-age') || {}).value, 10);
        sex = (document.getElementById('cooper-sex') || {}).value || 'male';
      ...
      case 'mile-run':
        ...
        sex = (document.getElementById('run-sex') || {}).value || 'male';
      ...
      case 'rockport':
        ...
        var walkWeightUnit = (document.getElementById('walk-weight-unit') || {}).value || 'lbs';
        var walkHR = parseInt((document.getElementById('walk-hr') || {}).value, 10);
        age = parseInt((document.getElementById('walk-age') || {}).value, 10);
        sex = (document.getElementById('walk-sex') || {}).value || 'male';
      ...
      case 'step':
        ...
        sex = (document.getElementById('step-sex') || {}).value || 'male';
      ...
      case 'hr-ratio':
        var hrMaxEl = document.getElementById('hr-max');
        var hrMax = hrMaxEl ? hrMaxEl.value : '';
        var hrRest = parseInt((document.getElementById('hr-rest') || {}).value, 10);
        age = parseInt((document.getElementById('hr-age') || {}).value, 10);
        sex = (document.getElementById('hr-sex') || {}).value || 'male';
        if (isNaN(age) || age < 15 || age > 100) { alert('Please enter your age.'); return; }
      ...
      case 'non-exercise':
        ...
        sex = (document.getElementById('ne-sex') || {}).value || 'male';
        var neWeight = parseFloat((document.getElementById('ne-weight') || {}).value);
        var neWeightUnit = (document.getElementById('ne-weight-unit') || {}).value || 'lbs';
        var neHeight = parseFloat((document.getElementById('ne-height') || {}).value);
        var neHeightUnit = (document.getElementById('ne-height-unit') || {}).value || 'in';
    ...
    // Display results
    var vo2ResultEl = document.getElementById('vo2-result');
    var vo2CategoryEl = document.getElementById('vo2-category');
    if (vo2ResultEl) vo2ResultEl.textContent = vo2.toFixed(1);
    if (vo2CategoryEl) {
      vo2CategoryEl.textContent = classification.label;
      vo2CategoryEl.style.color = classification.color;
    }
    ...
    var statusTextEl = document.getElementById('vo2-status-text');
    if (statusTextEl) statusTextEl.textContent = getStatusMessage(classification.label, vo2);

    // Summary boxes
    var compVo2 = document.getElementById('comp-vo2');
    var compCategory = document.getElementById('comp-category');
    var compFitnessAge = document.getElementById('comp-fitness-age');
    if (compVo2) compVo2.textContent = vo2.toFixed(1) + ' ml/kg/min';
    if (compCategory) {
      compCategory.textContent = classification.label;
      compCategory.style.color = classification.color;
    }
    if (compFitnessAge) compFitnessAge.textContent = fitAge < 75 ? '~' + fitAge + ' years' : '70+';

    // Breakdown
    var displayVo2 = document.getElementById('display-vo2');
    var displayCategory = document.getElementById('display-category');
    var displayFitnessAge = document.getElementById('display-fitness-age');
    var displayMethod = document.getElementById('display-method');
    var displayMets = document.getElementById('display-mets');
    if (displayVo2) displayVo2.textContent = vo2.toFixed(1) + ' ml/kg/min';
    if (displayCategory) {
      displayCategory.textContent = classification.label;
      displayCategory.style.color = classification.color;
    }
    if (displayFitnessAge) displayFitnessAge.textContent = fitAge < 75 ? '~' + fitAge : '70+';
    if (displayMethod) displayMethod.textContent = getMethodName(method);

    var mets = vo2 / 3.5;
    if (displayMets) displayMets.textContent = mets.toFixed(1) + ' METs';
    ...
    if (typeof animateValue === 'function' && vo2ResultEl) {
      animateValue(vo2ResultEl, 0, Math.round(vo2), 600);
    }
