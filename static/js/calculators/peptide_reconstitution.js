// Peptide Reconstitution Calculator — factory-compatible
(function() {
    var presets = {
        'custom': { mg: '', water: '' },
        'sema-2': { mg: 2, water: '' },
    ...
    if (btn) btn.addEventListener('click', function() {
        ...
        var shareText = 'Peptide dosing: ' + injectUnits.toFixed(1) + ' units (' + doseMg + ' mg from ' + peptideMg + ' mg/' + waterMl + ' mL)\n\nCalculate yours: healthcalculators.xyz/peptide-reconstitution-calculator';
        if (typeof updateShareButtons === 'function') updateShareButtons(shareText);

        if (typeof factoryReveal === 'function') factoryReveal();

        if (typeof hcTrackEvent === 'function') hcTrackEvent('calculator_complete', {calculator_name: 'Peptide Reconstitution Calculator', page_path: '/peptide-reconstitution-calculator'});
    });
})();
